// Lightweight Server-Sent Events (SSE) channel for attendance change notifications.
...
function broadcastAttendanceChanged(payload) {
  const id = ++seq;
  const data = {
  ...
  for (const client of clients) {
    const ok = writeEvent(client, { id, event: 'attendance.changed', data });
    if (!ok) clients.delete(client);
  }
}

function attachAttendanceStream(router) {
  router.get('/attendance/stream', (req, res) => {
    res.writeHead(200, {
