...
- **Phase A partial landing (2026-04-24):** [scripts/migrate-ccl-field-edits.mjs](../../scripts/migrate-ccl-field-edits.mjs) now exists, and [server/routes/ccl.js](../../server/routes/ccl.js) already records per-save field diffs into `CclFieldEdits` when the table exists. The migration has to be run in the Instructions DB before those writes become live data. A3 (`mark-sent`) is still outstanding.
- **Server file-shape warning:** [server/routes/ccl.js](../../server/routes/ccl.js) is already carrying generation, merge, draft persistence, and now field-diff helpers. If A3/A4 add much more logic, extract the feedback-loop helpers into a dedicated util rather than continuing to grow the route file.
- **Persistence module warning:** [server/utils/cclPersistence.js](../../server/utils/cclPersistence.js) now spans content, AI traces, assessments, status transitions, pressure-test persistence, and field-edit persistence. Keep adding to it only if the concern is truly shared; otherwise split feedback-loop reporting/query helpers before Phase B digest work turns it into a catch-all.
