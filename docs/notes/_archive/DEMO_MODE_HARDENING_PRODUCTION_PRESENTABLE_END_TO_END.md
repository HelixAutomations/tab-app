...
> ⚠️ **CLOSED 2026-05-06 — superseded by [`HELIX_REHEARSAL_RECORD_LUKE_TEST_AS_FIRM_SEED`](../HELIX_REHEARSAL_RECORD_LUKE_TEST_AS_FIRM_SEED.md).**
>
> Phases A/B/E of this brief were delivered in a different shape by the rehearsal-record work (Phase A + B). The still-relevant items (Clio dry-run safety, env-split ND folders, telemetry, runbook discoverability link) were folded into Phase C of the rehearsal-record brief. Per-user ND folders (old D2) and the standalone walkthrough page (old E2) were intentionally dropped. This file is kept in `_archive/` for history only — do **not** execute from it; execute from the rehearsal-record brief.
>
> ---
>
