...
const MatterReplayWorkbench: React.FC<MatterReplayWorkbenchProps> = ({ viewerInitials, isDarkMode, onBack, onOpenDashboard }) => {
  ...
  return (
    <section data-helix-region={rootRegion} style={{ color: textColour, fontFamily: 'Raleway, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 18 }}>
        ...
        {(onBack || onOpenDashboard) && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {onBack && <HeaderButton label="Back" isDarkMode={isDarkMode} onClick={onBack} />}
            {onOpenDashboard && <HeaderButton label="Dashboard" isDarkMode={isDarkMode} accent={colours.highlight} onClick={onOpenDashboard} />}
          </div>
        )}
      </div>
