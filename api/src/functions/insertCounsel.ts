...
interface InsertCounselRequest {
  prefix: string;
  first_name: string;
  last_name: string;
...
async function getRequestBody(req: HttpRequest): Promise<InsertCounselRequest> {
...
async function insertCounsel(data: InsertCounselRequest, config: any): Promise<InsertResult> {
  return new Promise<InsertResult>((resolve, reject) => {
    ...
    connection.on('connect', err => {
      ...
      const q = `INSERT INTO dbo.counsel_recommendations 
        (submitted_by, prefix, first_name, last_name, chambers_name, email, clerks_email, phone, website, area_of_work, worktype, price_tier, introduced_by, source, notes)
        OUTPUT Inserted.id
        VALUES (@SubmittedBy, @Prefix, @FirstName, @LastName, @ChambersName, @Email, @ClerksEmail, @Phone, @Website, @AreaOfWork, @Worktype, @PriceTier, @IntroducedBy, @Source, @Notes);`;
      ...
      r.addParameter('ChambersName', TYPES.NVarChar, data.chambers_name || null);
      r.addParameter('Email', TYPES.NVarChar, data.email || null);
      r.addParameter('ClerksEmail', TYPES.NVarChar, data.clerks_email || null);
      ...
      r.addParameter('PriceTier', TYPES.NVarChar, data.price_tier || 'mid');
      ...
      r.on('requestCompleted', () => {
        connection.close();
        resolve({ success: true, message: 'Counsel recommendation saved successfully', insertedId });
      });
...
export async function insertCounselHandler(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  context.log('insertCounsel function triggered');
  ...
  try {
    ...
    context.log('Received counsel data:', { first_name: data.first_name, last_name: data.last_name, area_of_work: data.area_of_work });
    ...
    if (!data.email?.trim()) {
      return {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Email is required for counsel recommendations' }),
      };
    }
    if (!data.price_tier?.trim()) {
      return {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Price tier is required' }),
      };
    }
    ...
    const result = await insertCounsel(data, config);

    context.log('Counsel recommendation saved successfully:', result);
    return {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    context.log('ERROR inserting counsel recommendation:', errorMessage);
    return {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
...
app.http('insertCounsel', {
  methods: ['GET', 'POST', 'OPTIONS'],
  authLevel: 'anonymous',
  route: 'counsel',
  handler: insertCounselHandler,
});
